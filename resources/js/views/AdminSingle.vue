<template>
  <div>
    ...
    <div class="form-group row">
      <label for="user_name" class="col-md-12 col-form-label text-md-right"
        >Current Password Access</label
      >
      <div class="col-md-12">
        <table style="width: 100%">
          ...
          <tr v-for="(password, index) in user.passwords" :key="index">
            ...
            <td>{{ password.name }}</td>
            <td>
              <button @click="removePassword(password.id)">
                Remove Password
              </button>
            </td>
          </tr>
      ...
      <div class="col-md-12">
        ...
        <select v-model="passwordToAdd">
          <option
            v-for="(password, index) in allPasswordsExcludingActive"
            :key="index"
            :value="password.id"
          >
            {{ password.name }}
...
export default {
  ...
  methods: {
    getUser() {
      axios
        .get(GET_USER(this.$route.params.user))
        .then(({ data: { data } }) => {
          this.user = data;
        });
    },
    ...
    removePassword(passwordId) {
      axios
        .post(REMOVE_PASSWORD_FROM_USER, {
          user: this.user.id,
          password: passwordId,
        })
        .then((res) => {
